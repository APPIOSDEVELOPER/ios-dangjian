...
{
    CGPoint characterPoint;
    
    NSMutableArray *allWordsArry;
...
//    [_linkedTextDefaultAttributeDictionary removeAllObjects];
//    [_linkedTextHighlightAttributeDictionary removeAllObjects];
    
    if (!string || !defaultAttributes || !highlightedAttributes) {
        return;
    }
...
- (void)linkString:(NSString *)string defaultAttributes:(NSDictionary *)defaultAttributes highlightedAttributes:(NSDictionary *)highlightedAttributes isHiglighted:(BOOL)highlited tapHandler:(LinkedStringTapHandler)tapHandler {
...
//    [_linkedTextHighlightAttributeDictionary removeAllObjects];
    ...
    NSDictionary *linkedStringDictionary = [self dictionaryForLinkedString:string
                                                     ...
                                                     highlightedAttributes:highlightedAttributes
                                                                tapHandler:tapHandler];
    NSMutableDictionary *dict = linkedStringDictionary.mutableCopy;
    dict[@"isHiglighted"] = @(highlited);
    if (self.linkedStringsAndAttributes.count > 0) {
        [self addLinkedStringAndAttributes:dict];
    }else{
        [self setLinkedStringsAndAttributes:@[dict].mutableCopy];
    }
...
{
    [allWordsArry removeAllObjects];
   
    if (!strings || !defaultAttributes || !highlightedAttributes) {
        return;
    }
//    [_linkedTextDefaultAttributeDictionary removeAllObjects];
//    [_linkedTextHighlightAttributeDictionary removeAllObjects];
...
- (void)addLinkedStringAndAttributes:(NSDictionary *)linkedStringDictionary
{
    ...
    [self checkString:string fromText:text length:0];
    if (rangeOfString.length) {
        ...
        BOOL hilghted = [linkedStringDictionary[@"isHiglighted"] boolValue];
        if (hilghted) {
            [mutableAttributedString addAttributes:linkedStringDictionary[@"highlightedAttributes"] range:rangeOfString];
...
- (void)setLinkedStringsAndAttributes:(NSMutableArray *)linkedStringsAndAttributes
{
    ...
    BOOL highlited = [linkedStringsAndAttributes.firstObject[@"isHiglighted"] boolValue];
...
- (void)checkString:(NSString*)str fromText:(NSString*)text length:(NSUInteger)currentIndex{
    
    
    
    
    NSRange range = [text rangeOfString:str options:NSLiteralSearch];
    if (range.length) {
        NSValue *value = [NSValue valueWithRange:NSMakeRange(range.location + currentIndex, range.length)];
        [allWordsArry addObject:@{str:value}];

        NSString *subStrl = [text substringFromIndex:NSMaxRange(range)];
        [self checkString:str fromText:subStrl length:NSMaxRange(range)];
        
    }
    
//    NSArray *allwords = [text componentsSeparatedByString:@" "];
//    [allwords enumerateObjectsUsingBlock:^(NSString*  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
//        
//        if ([obj isEqualToString:str]) {
//            NSRange range = NSMakeRange(idx, [obj length]);
//            NSValue *value = [NSValue valueWithRange:range];
//            [allWordsArry addObject:@{obj:value}];
//        }
//        
//    }];
}
...
- (NSString *)findLinkedStringAtPoint:(CGPoint)point inDictionary:(NSDictionary *)dictionary
{
    ...
    NSUInteger characterIndex = [self.layoutManager
                      ...
                      inTextContainer:self.textContainer fractionOfDistanceBetweenInsertionPoints:&offsetY];
...
//    NSLog(@"effory = %@",NSStringFromCGRect(rect));
//    
//    CGPoint charPoint = [self.layoutManager locationForGlyphAtIndex:glyphAtIndex];
//    
//    CGPoint bRect = [self.layoutManager locationForGlyphAtIndex:glyphAtIndex];
//    
//    CGSize characterSize = [self.layoutManager attachmentSizeForGlyphAtIndex:glyphAtIndex];
//    
//    NSLog(@"charize = %@",NSStringFromCGSize(characterSize));
//    NSLog(@"charPoint = %@",NSStringFromCGPoint(charPoint));

//    characterPoint = rect.origin;//CGPointMake(charPoint.x + characterSize.width / 2, charPoint.y + characterSize.height / 2);
    ...
    if (result == nil) {
        [allWordsArry enumerateObjectsUsingBlock:^(NSDictionary*  _Nonnull dict, NSUInteger idx, BOOL * _Nonnull stop) {
            
            NSValue *value = dict.allValues.firstObject;
    ...
    if (result == nil) {
//        NSString *news = [self.text substringFromIndex:characterIndex];
//        if (news cha) {
//            <#statements#>
//        }
...
- (void)setShowDefultDict{
    
    selRange = NSMakeRange(0, 0);
    
    [_linkedTextDefaultAttributeDictionary enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
       
        NSMutableAttributedString *defultAttribute = self.attributedText.mutableCopy;
        [defultAttribute beginEditing];
        [defultAttribute setAttributes:obj range:NSMakeRange(0, self.text.length)];
        [defultAttribute endEditing];
        self.attributedText = defultAttribute;
...
- (void)tapGesture:(UITapGestureRecognizer*)tap{
    CGPoint pointer = [tap locationInView:self];
    [self setShowDefultDict];
    
    CGPoint location = pointer;
    location.x -= self.textContainerInset.left;
    location.y -= self.textContainerInset.top;
    self.initialTouchLocation = [NSValue valueWithCGPoint:location];
    
    NSString *tappedString = [self findLinkedStringAtPoint:location inDictionary:self.linkedTextRangeDictionary];
    ...
    if (tappedString && highlightAttributes) {
        ...
        if (tapHandler) {
            tapHandler(tappedString,pointer);
        }
    }
...
//            
//            characterPoint = [touches.anyObject locationInView:self];
//            
//        }
//        self.activeString = nil;
//    }
...
- (void)touchesCancelled:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (tappedString && defaultAttributes) {
        ...
        if ([self continueTrackingTouches:touches]) {
            ...
            if (tapHandler) {
                tapHandler(tappedString,characterPoint);
            }
        }
        self.activeString = nil;
    }
