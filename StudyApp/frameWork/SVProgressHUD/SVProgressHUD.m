...
- (void)showProgress:(float)progress status:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType {
    ...
    if(self.maskType != SVProgressHUDMaskTypeNone) {
        ...
        self.accessibilityLabel = [string stringByReplacingOccurrencesOfString:@"..." withString:@""];
        self.isAccessibilityElement = YES;
    }
    else {
        self.overlayView.userInteractionEnabled = NO;
        self.hudView.accessibilityLabel = [string stringByReplacingOccurrencesOfString:@"..." withString:@""];;
...
- (void)showImage:(UIImage *)image status:(NSString *)string duration:(NSTimeInterval)duration {
    ...
    self.fadeOutTimer = [NSTimer timerWithTimeInterval:duration+0.3 target:self selector:@selector(dismiss) userInfo:nil repeats:NO];
...
#if __IPHONE_OS_VERSION_MIN_REQUIRED >= 70000
                             // Tell the KCRootViewController to update the StatusBar appearance
...
+ (void)setBackgroudBlackColor{

    [[SVProgressHUD sharedView]setHudBackgroundColor:[[UIColor blackColor] colorWithAlphaComponent:0.8]];
    [[SVProgressHUD sharedView]setHudForegroundColor:[UIColor whiteColor]];

}
+ (void)setDefualtColor{
    [[SVProgressHUD sharedView]setHudBackgroundColor:[UIColor whiteColor]];
    [[SVProgressHUD sharedView]setHudForegroundColor:[UIColor blackColor]];
